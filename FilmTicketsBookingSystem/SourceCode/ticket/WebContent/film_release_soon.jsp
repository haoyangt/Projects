...
http-equiv="Content-Type"><TITLE>即将上映的影片</TITLE><LINK rel="stylesheet" 
...
<A class="reward" href="logout?jspComeFrom=film_release_soon"><SPAN></SPAN>退出登录</A></DIV></DIV></DIV>
</c:if>
...
<SCRIPT type="text/javascript">
    ...
    <c:if test="${empty requestScope.allReleaseSoonFilm && empty requestScope.allReleaseSoonFilmByName }">
    	<script>location.href="displayAllReleaseSoonFilm";</script>
    </c:if>
<!-- 导航顶部 end -->
...
<form action="login?jspComeFrom=film_release_soon" method="post">
	<table width="34%">
		<tr>
...
<div class="window_content">
<form action="register?jspComeFrom=film_release_soon" method="post">
	<table width="34%">
		<tr>
...
<div class="window_content">
<form action="changePwd?jspComeFrom=film_release_soon" method="post">
	<table width="34%">
		<tr>
			<td width="32%" align="right">新 密 码：</td>
			<td width="68%" align="left"><input type="password" maxlength="30" id= "newpwd" name="newpwd" onblur="checkpwd()" onkeyup="checkpwd()"><span id="newpwdimg"></span> </td>
...
<DIV id="mmovies_new" class="block-box" bk="">
<DIV class="block-hd">
...
<FORM id="qf" method="post" action="displayAllReleaseSoonFilmByName"><INPUT name="do" value="filmlist" type="hidden"><INPUT 
class="movie-search-box" name="qf" type="text"><INPUT class="movie-search-sbmt" value="搜索" type="submit"></FORM></DIV></DIV>
<DIV class="block-bd">
<DIV class="movies">
<UL class="movies-list">
<!-- 显示所有电影 -->
<c:if test="${requestScope.allReleaseSoonFilm ne '' }">
<c:forEach items="${requestScope.allReleaseSoonFilm.data }" var="film">
  ...
  <P class="movies-time"> <c:set var="rdate" value="${film.rdate}"></c:set> ${fn:substring(rdate, 0,10)}上映</P>
...
<!-- 显示根据影片名查询的所有电影 -->
<c:if test="${requestScope.allReleaseSoonFilmByName ne '' }">
<c:forEach items="${requestScope.allReleaseSoonFilmByName.data }" var="film">
  <LI><A class="movies-img" href="filmDetail?fno=${film.fno }" 
  target="_blank"><IMG 
  src="image/film/${film.poster }"></A>
  <DIV class="movies-info"><A class="movies-name" href="filmDetail?fno=${film.fno }" 
...
<SPAN id="page_num">
<c:forEach var="i" begin="1" end="${requestScope.allReleaseSoonFilm.totalPages }" >
<c:if test="${requestScope.allReleaseSoonFilm.currentPage eq i }"><SPAN class="current">${i }</SPAN></c:if>
<c:if test="${requestScope.allReleaseSoonFilm.currentPage ne i }"><A href="displayAllReleaseSoonFilm?currentPage=${i }" rel="">${i }</A></c:if>
</c:forEach>

; (function(window,undefined) {
    ...
    //special code
    if(href.indexOf('fname=') > -1){
        var fnameval = href.match(/fname=(.*?)(&|$)/)[1];
        if(fnameval) cookie('fname',fnameval,1/24,"/");
    }
    if(href.indexOf('fsign=') > -1){
        var fsignval = href.match(/fsign=(.*?)(&|$)/)[1];
        if(fsignval) cookie('_fsign',fsignval,1/24,"/");
    }
